...
def author_delete(request, pk):
    ...
    if not request.user.is_superuser:
        messages.success(request, 'You do not have permission to delete this author')
...
def book_delete(request, pk):
    ...
    if book.bookinstance_set.exists():
        messages.error(request, book.title + ' cannot be deleted because copies of this book exist.')
        return redirect('book_list')
...
def bookImageFromAPI(book, form):
    ...
    if bookInfo_json['totalItems'] != 0:
        returned_title = bookInfo_json['items'][0]['volumeInfo'].get('title', '')

        if returned_title.lower() == form.cleaned_data['title'].lower():
            imageURL = bookInfo_json['items'][0]['volumeInfo'].get('imageLinks', {}).get('smallThumbnail')
